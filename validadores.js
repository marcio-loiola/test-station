 ...
 */
function validarEmailDuplicado(email, usuariosExistentes = [], caseSensitive = false) {
    if (!email || typeof email !== 'string') return false;
    const emailLimpo = email.trim();
    if (caseSensitive) {
        return usuariosExistentes.some(u => u.email === emailLimpo);
    }
    return usuariosExistentes.some(u => (u.email || '').toLowerCase() === emailLimpo.toLowerCase());
}
